;(function($,_scope){'use strict';_scope=_scope===undefined?window:_scope;if(_scope._pickyImg)
return;$=$;var _w=window,_pickyImg=(function(){var ME={},UA=_scope._UAjammer!==undefined?_scope._UAjammer:undefined,args,pad,wChangeTimer,wIsStopped=true,whichSrc,srcMap,selector,origin={},callbacks={},bindMe,binder,wStopped,isFinished,defMap;ME.doCallback=function(event){var func=callbacks[event];if(func!==undefined&&typeof callbacks[event]=='function')
func();};ME.init=function(initArgs){if(!$().inView){$.fn.inView=function(j){var h=false,g=$(window);if(this.length>0){var i=this.offset()===null?0:this.offset().top,f=i+this.height();j=j===undefined?g.height():j;h=((g.scrollTop()+g.height())+j)>=i&&(g.scrollTop()-j)<=f?true:false}return h}};args=initArgs!==undefined?initArgs:{};pad=args.pad!==undefined?args.pad:_w.outerHeight;whichSrc=args.whichSrc!==undefined?args.whichSrc:'data-src-default';srcMap=args.srcMap!==undefined?args.srcMap:undefined;selector=args.selector!==undefined?args.selector:'._picky';bindMe=args.bindMe!==undefined?args.bindMe:true;if(args.callbacks===undefined)
...
return chosenSrc;};binder=function(){$(_w).on('scroll',function(){wIsStopped=false;clearTimeout(wChangeTimer);wChangeTimer=setTimeout(wStopped,150);});$(document.body).on('wStopped',selector+":not(._picky_picked)",function(){ME.doCallback('picking');$(_w).trigger('picky_picking');$('body').removeClass('_picky_finished');ME.pickMe($(this));});wStopped();};wStopped=function(){wIsStopped=true;$(_w).trigger('wStopped');$('body '+selector).trigger('wStopped');};ME.pickMe=function(dummy){if(dummy.inView(pad)&&!dummy.hasClass('_picky_picking')){dummy.addClass("_picky_picking");var no_script=dummy.find('noscript'),src=dummy.attr(whichSrc),img=new Image(),imgClass=dummy.attr('data-class'),imgAlt=dummy.attr('data-alt'),imgLoad_handler;imgLoad_handler=function(){if(dummy.hasClass('_picky_bg')){dummy.css({'background-image':'url('+src+')'});}else{dummy.prepend($(this));}
...
dummy.attr('data-pref-src',src);img.src=src;}};isFinished=function(){if($(selector+":not(._picky_picked)").length<=0){$('body').addClass('_picky_finished');$(_w).trigger('picky_finished');ME.doCallback('finished');}};return ME;})();_scope._pickyImg=_pickyImg;})((jQuery||$),(typeof scope=='undefined'?window:scope));if(typeof define==="function"){define("_pickyImg",[],function(){return _pickyImg;});}